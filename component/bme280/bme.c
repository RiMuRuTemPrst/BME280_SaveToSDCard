...
BME280_OVERSAMP_16X: Value is 0x05, 0x06 and 0x07
    */
    ...
    /**
     * @brief After that we will write the standby duration time from the sensor in the register 0xF5 bit 5 to 7 by calling bme280_set_standby_durn(). 
     * This function takes in a single parameter which is the value of the standby duration time. It can take any value listed below:
...
BME280_STANDBY_TIME_4000_MS
    ...
    if (com_rslt == SUCCESS)
    {
        ...
        while(true)
        {
            ...
            if (com_rslt == SUCCESS)
            {
                my_data.temperature = bme280_compensate_temperature_double(v_uncomp_temperature_s32);
                my_data.humidity = bme280_compensate_humidity_double(v_uncomp_humidity_s32);
                my_data.pressure = bme280_compensate_pressure_double(v_uncomp_pressure_s32)/100;
